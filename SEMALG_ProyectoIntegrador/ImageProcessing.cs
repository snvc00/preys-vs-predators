...
namespace SEMALG_ProyectoIntegrador
{
    public class ImageProcessing
    {
                ...
                for (int i = 0; i < ICircleList.Count - 1; ++i)
                {
                    for (int j = i + 1; j < ICircleList.Count; ++j)
                    {
                        if (IsObstacleFree(IGraph[i].Circle, IGraph[j].Circle))
                        {
                            ...
                            if (ToolFunction.Distance(IGraph[i].GetCirclePoint(), Coords[0]) < 5)
                            {
                                IGraph[i].AddEdge(new Edge(distance, IGraph[j], new List<Point>(this.Coords), IGraph[i].ID.ToString()));
                                this.Coords.Reverse();
                                IGraph[j].AddEdge(new Edge(distance, IGraph[i], new List<Point>(this.Coords), IGraph[j].ID.ToString()));

                            }
                            else
                            {
                                IGraph[j].AddEdge(new Edge(distance, IGraph[i], new List<Point>(this.Coords), IGraph[i].ID.ToString()));
                                this.Coords.Reverse();
                                IGraph[i].AddEdge(new Edge(distance, IGraph[j], new List<Point>(this.Coords), IGraph[j].ID.ToString()));
                            }
        ...
        private void ModifyBitmapFromGraph()
        {
            ...
            Font font = new Font("Roboto", fontSize, FontStyle.Regular);
            ...
            Bitmap shield = new Bitmap("../../../resources/mainwindow/shield.png");

            foreach (Vertex v in IGraph.VertexList)
            {
                Graphics.FromImage(IBmp).DrawImage(shield, (float)(v.GetCirclePoint().X - (v.Circle.Radius * 2.5)), (float)(v.GetCirclePoint().Y - (v.Circle.Radius * 2.5)), v.Circle.Radius * 5, v.Circle.Radius * 5);
                Graphics.FromImage(IBmp).DrawString(v.ID.ToString(), font, new SolidBrush(fontColor), v.GetCirclePoint().X, v.GetCirclePoint().Y, strformat);
            }
        }
